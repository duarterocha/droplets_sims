    ...
    double _res_contrib,_J_contrib;
    {
      ...
      for (unsigned int l_test=0;l_test<eleminfo->nnode_C2;l_test++)
      {
        ...
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind__kin_bc],6.2831853071795862e+00*dx*this_intrp_d0t0_d0x_coordinate_x*testfunction[l_test]*( shapeinfo->normal[ipt][0]*this_intrp_d0t0_d0x_velocity_x+shapeinfo->normal[ipt][1]*this_intrp_d0t0_d0x_velocity_y), shapeinfo->hanginfo_C2,this_nodalind__kin_bc,l_test)
          ...
          END_JACOBIAN()
        END_RESIDUAL_CONTINUOUS_SPACE()
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind_velocity_x],-2.0000000000000000e+00*( 3.1415926535897931e+00*shapeinfo->normal[ipt][0]*this_intrp_d0t0_d0x__kin_bc*this_intrp_d0t0_d0x_coordinate_x*testfunction[l_test]+3.1415926535897931e+00*( 1.0/this_intrp_d0t0_d0x_coordinate_x*testfunction[l_test]+dx_testfunction[l_test][0])*(*(my_func_table->global_parameters[0]))*this_intrp_d0t0_d0x_coordinate_x*this_intrp_d0t0_d0x_T)*dx, shapeinfo->hanginfo_C2,this_nodalind_velocity_x,l_test)
          ADD_TO_RESIDUAL_CONTINUOUS_SPACE()
          BEGIN_JACOBIAN()
            for (unsigned int l_shape=0;l_shape<eleminfo->nnode_C2;l_shape++)
            {
...
static void dResidual0dParameter_0(const JITElementInfo_t * eleminfo, const JITShapeInfo_t * shapeinfo,double * residuals, double *jacobian, double *mass_matrix,unsigned flag)
{
 ...
 //Subexpressions // TODO: Check whether it is constant to take it out of the loop
    ...
    double _res_contrib,_J_contrib;
    {
      ...
      for (unsigned int l_test=0;l_test<eleminfo->nnode_C2;l_test++)
      {
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind_velocity_x],-6.2831853071795862e+00*( 1.0/this_intrp_d0t0_d0x_coordinate_x*testfunction[l_test]+dx_testfunction[l_test][0])*dx*this_intrp_d0t0_d0x_coordinate_x*this_intrp_d0t0_d0x_T, shapeinfo->hanginfo_C2,this_nodalind_velocity_x,l_test)
          ADD_TO_RESIDUAL_CONTINUOUS_SPACE()
          BEGIN_JACOBIAN()
            for (unsigned int l_shape=0;l_shape<eleminfo->nnode_C2;l_shape++)
            {
              BEGIN_JACOBIAN_HANG(eleminfo->nodal_local_eqn[l_shape][this_nodalind_T], -6.2831853071795862e+00*( 1.0/this_intrp_d0t0_d0x_coordinate_x*testfunction[l_test]+dx_testfunction[l_test][0])*dx*this_intrp_d0t0_d0x_coordinate_x*shapeinfo->shape_C2[ipt][l_shape],shapeinfo->hanginfo_C2,this_nodalind_T,l_shape)
                ADD_TO_JACOBIAN_HANG_HANG()
              END_JACOBIAN_HANG()
            }
          END_JACOBIAN()
