...
static void ResidualAndJacobian0(const JITElementInfo_t * eleminfo, const JITShapeInfo_t * shapeinfo,double * residuals, double *jacobian, double *mass_matrix,unsigned flag)
{
 ...
 //Subexpressions // TODO: Check whether it is constant to take it out of the loop
    ...
    double _res_contrib,_J_contrib;
    {
      ...
      {
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind_T],-6.2831853071795862e+00*dx*this_intrp_d0t0_d0x_coordinate_x*( shapeinfo->normal[ipt][0]*oppblk_intrp_d0t0_d1x0_c+shapeinfo->normal[ipt][1]*oppblk_intrp_d0t0_d1x1_c)*testfunction[l_test], shapeinfo->hanginfo_C2,this_nodalind_T,l_test)
          ADD_TO_RESIDUAL_CONTINUOUS_SPACE()
          BEGIN_JACOBIAN()
            for (unsigned int l_shape=0;l_shape<eleminfo->opposite_eleminfo->bulk_eleminfo->nnode_C2;l_shape++)
            {
              BEGIN_JACOBIAN_HANG(eleminfo->opposite_eleminfo->bulk_eleminfo->nodal_local_eqn[l_shape][oppblk_nodalind_c], -6.2831853071795862e+00*dx*this_intrp_d0t0_d0x_coordinate_x*( shapeinfo->normal[ipt][0]*shapeinfo->opposite_shapeinfo->bulk_shapeinfo->dx_shape_C2[ipt][l_shape][0]+shapeinfo->normal[ipt][1]*shapeinfo->opposite_shapeinfo->bulk_shapeinfo->dx_shape_C2[ipt][l_shape][1])*testfunction[l_test],shapeinfo->opposite_shapeinfo->bulk_shapeinfo->hanginfo_C2,oppblk_nodalind_c,l_shape)
                ADD_TO_JACOBIAN_HANG_HANG()
              END_JACOBIAN_HANG()
            }
          END_JACOBIAN()
        END_RESIDUAL_CONTINUOUS_SPACE()
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind__kin_bc],6.2831853071795862e+00*dx*this_intrp_d0t0_d0x_coordinate_x*( shapeinfo->normal[ipt][0]*this_intrp_d0t0_d0x_velocity_x+shapeinfo->normal[ipt][1]*this_intrp_d0t0_d0x_velocity_y)*testfunction[l_test], shapeinfo->hanginfo_C2,this_nodalind__kin_bc,l_test)
          ADD_TO_RESIDUAL_CONTINUOUS_SPACE()
          BEGIN_JACOBIAN()
            for (unsigned int l_shape=0;l_shape<eleminfo->nnode_C2;l_shape++)
            {
