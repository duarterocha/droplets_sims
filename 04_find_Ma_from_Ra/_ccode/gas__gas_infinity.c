...
static void ResidualAndJacobian0(const JITElementInfo_t * eleminfo, const JITShapeInfo_t * shapeinfo,double * residuals, double *jacobian, double *mass_matrix,unsigned flag)
{
 ...
 //Subexpressions // TODO: Check whether it is constant to take it out of the loop
    ...
    double _res_contrib,_J_contrib;
    {
      ...
      {
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind_c],6.2831853071795862e+00*dx/( pow(this_intrp_d0t0_d0x_coordinate_x,2.0)+pow(this_intrp_d0t0_d0x_coordinate_y,2.0))*this_intrp_d0t0_d0x_coordinate_x*this_intrp_d0t0_d0x_c*( shapeinfo->normal[ipt][0]*this_intrp_d0t0_d0x_coordinate_x+shapeinfo->normal[ipt][1]*this_intrp_d0t0_d0x_coordinate_y)*testfunction[l_test], shapeinfo->hanginfo_C2,this_nodalind_c,l_test)
          ADD_TO_RESIDUAL_CONTINUOUS_SPACE()
          BEGIN_JACOBIAN()
            for (unsigned int l_shape=0;l_shape<eleminfo->nnode_C2;l_shape++)
            {
              BEGIN_JACOBIAN_HANG(eleminfo->nodal_local_eqn[l_shape][this_nodalind_c], 6.2831853071795862e+00*dx/( pow(this_intrp_d0t0_d0x_coordinate_x,2.0)+pow(this_intrp_d0t0_d0x_coordinate_y,2.0))*this_intrp_d0t0_d0x_coordinate_x*shapeinfo->shape_C2[ipt][l_shape]*( shapeinfo->normal[ipt][0]*this_intrp_d0t0_d0x_coordinate_x+shapeinfo->normal[ipt][1]*this_intrp_d0t0_d0x_coordinate_y)*testfunction[l_test],shapeinfo->hanginfo_C2,this_nodalind_c,l_shape)
