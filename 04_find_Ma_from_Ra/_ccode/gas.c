...
static void ResidualAndJacobian0(const JITElementInfo_t * eleminfo, const JITShapeInfo_t * shapeinfo,double * residuals, double *jacobian, double *mass_matrix,unsigned flag)
{
 ...
 //Subexpressions // TODO: Check whether it is constant to take it out of the loop
    ...
    double _res_contrib,_J_contrib;
    {
      ...
      {
        BEGIN_RESIDUAL_CONTINUOUS_SPACE(eleminfo->nodal_local_eqn[l_test][this_nodalind_c],6.2831853071795862e+00*dx*( this_intrp_d0t0_d1x0_c*dx_testfunction[l_test][0]+this_intrp_d0t0_d1x1_c*dx_testfunction[l_test][1])*this_intrp_d0t0_d0x_coordinate_x, shapeinfo->hanginfo_C2,this_nodalind_c,l_test)
          ADD_TO_RESIDUAL_CONTINUOUS_SPACE()
          BEGIN_JACOBIAN()
            for (unsigned int l_shape=0;l_shape<eleminfo->nnode_C2;l_shape++)
            {
              BEGIN_JACOBIAN_HANG(eleminfo->nodal_local_eqn[l_shape][this_nodalind_c], 6.2831853071795862e+00*dx*( shapeinfo->dx_shape_C2[ipt][l_shape][0]*dx_testfunction[l_test][0]+shapeinfo->dx_shape_C2[ipt][l_shape][1]*dx_testfunction[l_test][1])*this_intrp_d0t0_d0x_coordinate_x,shapeinfo->hanginfo_C2,this_nodalind_c,l_shape)
