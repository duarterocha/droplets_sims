...
class DropletTempProblemMafromRa(DropletTempProblem): # Inherit from the previous problem
    ...
    def actions_after_newton_step(self):
        ...
        print("Currently at Ma=",self.get_Ma(),"Ra=",self.get_Ra(),"POSITIVE COVERED AREA",pos_area,"DESIRED",self.positive_stream_volume_fraction)
        # Set the Ma parameter (which is unused for the solution here) to the current value
        # it is necessary for the output
        self._param_Ma.value=self.get_Ma()
    ...
    def define_problem(self):
        ...
        smooth_heaviside=lambda x : (tanh(x*100000.0)+1)/2
...
with DropletTempProblemMafromRa() as problem:
    ...
    parameter_curve_file.write("\t".join(map(str, [problem.get_Ma(), problem.get_Ra()])) + "\n")
    ...
    with problem.select_dofs() as dofs:
        ...
        while problem.get_Ra() < 100000:  # Limit Ma
